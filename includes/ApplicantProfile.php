...
class ApplicantProfile {
    protected static $tablename = "applicant_profile";
    protected static $db_fields = array('id', 'userid', 'user_address', 'user_contact', 'gender', 'experience', 'educ_attainment', 'birthdate', 'cover_letter', 'file_name', 'status', 'jobid', 'created_at');
    public $id;
    public $userid;
    public $user_address;
    public $user_contact;
    ...
    public static function find_all_active_status() {
        global $database;
        $sql = "select * from " . self::$tablename . " WHERE NOT status = 100";
        $query = $database->query($sql);
        $query_in_array_form = array();
        while ($row = mysqli_fetch_array($query)) {
            $query_in_array_form[] = self::instantiate($row);
