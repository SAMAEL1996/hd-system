...
class Schedule {
    ...
    protected static $db_fields = array('id', 'clientid', 'action', 'sched_date');
    public $id;
    public $clientid;
    public $action;
    public $sched_date;
    ...
    public static function find_by_clientid($clientid = 0) {
        global $database;
        $query = self::find_by_sql("select * from " . self::$tablename . " where clientid ='" . $database->escape_value($clientid) . "' LIMIT 1");
        return !empty($query) ? array_shift($query) : false;
    }
